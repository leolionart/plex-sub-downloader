...
4. Tìm subtitle trên Subsource:
   - ưu tiên IMDb ID của movie/show từ Plex
   - nếu thiếu ID ngoài: fallback sang title/year/season scoring
   - trong các candidate hợp lệ: ưu tiên release name gần với filename video thật
5. Download → nếu là season pack ZIP thì chọn file con khớp episode tốt nhất → upload lên Plex
...
Service không chỉ sort subtitle theo quality/download count mà còn cố giảm nhầm media/sub release:

- **IMDb-first media lookup**: nếu Plex cung cấp IMDb ID cho movie/show, Subsource lookup đi theo ID này trước.
- **Title fallback có scoring**: title/year/season được chấm điểm để tránh lấy nhầm show cùng tên hoặc reboot khác năm.
- **Episode format parsing**: hiểu nhiều dạng như `S04E05`, `4x05`, `season-4-ep-5`, `first season`.
- **Release-aware ranking**: so khớp `video filename` với `release name` theo token release (`AMZN`, `WEB-DL`, `BluRay`, codec, resolution, v.v.).
- **ZIP member selection**: với season pack, chọn file khớp episode cần thiết hoặc filename video gần nhất thay vì file `.srt` đầu tiên.
